function deidentify(datasetPath, summaryFilesFolder)
summaryFilesFolder = fullfile(genpath(cd),'Summary');
...
for i = 1 : length(subDirs)
    ...
    %                     rectangle('Position', [270 240 130 80], 'FaceColor', 'Black');
        ...
        if not(isfolder(strcat(datasetPath, newCurDir)))
            mkdir(strcat(datasetPath, newCurDir))
        end
        ...
        if not(isfolder(strcat(datasetPath , newCurDir , '/' , char(mainDir))))
            mkdir(fullfile(datasetPath , newCurDir , '/' , char(mainDir)))
        end
        
        % Move file into its proper folder or create it and then move it
        if isfolder(strcat(datasetPath , newCurDir , '/' , char(mainDir) , '/' , char(subDir)))
            newPath = strcat(fullfile(datasetPath , newCurDir , '/' , char(mainDir) , '/' , char(subDir) , '/' , files(k).name), '.dcm');
            ...
            mkdir(fullfile(strcat(datasetPath , newCurDir , '/' , char(mainDir) , '/' , char(subDir))))
            newPath = strcat(fullfile(datasetPath , newCurDir , '/' , char(mainDir) , '/' , char(subDir) , '/' , files(k).name), '.dcm');
            gen_count = 0; err_count = 0;
            while gen_count == err_count
                try
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Delete The Old Folders and Files%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This can be done during the inner loop however empty folders will not be deleted.

%for curDir in subDirs:
    %print('--------------------------------------Deleting Directory %s------------------------------------' % curDir)
    %delete(datasetPath + curDir)
    %print('--------------------------------------Done Deleting Directory %s-------------------------------' % curDir)
